...
function ChatbotSection() {
  ...
  useEffect(() => {
    ...
    // Walk up the DOM to find the nearest scrollable ancestor (the phone's scroll container)
    ...
    const check = () => {
      ...
      // Fire when the top of ChatbotSection enters the scroll container's visible area
      if (elRect.top < scrollerRect.bottom) {
        setIsVisible(true);
    ...
    check(); // also check immediately in case section is already in view on mount
  ...
  return (
    <div
      ref={containerRef}
      className={`flex flex-col px-4 pt-6 pb-4 flex-1 min-h-[812px]${isVisible ? " chatbot-visible" : ""}`}
      style={{ background: "linear-gradient(179.83deg, rgb(239,233,251) 0%, rgb(248,247,250) 23.833%)" }}
    >
      {/* Drag handle */}
      <div className="flex justify-center mb-4 chatbot-child chatbot-d0">
